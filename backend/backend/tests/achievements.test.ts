 ...
 *
 * Mocks Prisma to test the `evaluateAchievements` logic independently.
 * Run with: npx ts-node tests/achievements.test.ts
 */
...
// ── Inline the evaluation logic (mirrors achievementService) ────

async function evaluateAchievements(mockPrisma: any, userId: string): Promise<string[]> {
  ...
  // Streaks
  if (habits.some((h: any) => h.streak >= 7) && !unlockedCodes.has("STREAK_7")) codesToUnlock.push("STREAK_7");
  if (habits.some((h: any) => h.streak >= 30) && !unlockedCodes.has("STREAK_30")) codesToUnlock.push("STREAK_30");
...
async function runTests() {
  ...
  // 1. No achievements with no activity
